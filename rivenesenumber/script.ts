...
class Digit {
    ...
    draw(svgParent: SVGElement) {
        const svgDigitContainer = <SVGElement><unknown>document.getElementById("digit-container").cloneNode(true);
        const svgDigit = <SVGClipPathElement><unknown>svgDigitContainer.children.item(0);
...
function onBodyLoad() {
    const inputField = <HTMLInputElement><unknown>document.getElementById("input-field");
    inputField.addEventListener("input", event => {
        currentValue = parseInt(inputField.value) ? parseInt(inputField.value) : 0;
        refresh();
    });
    const sizeField = <HTMLInputElement><unknown>document.getElementById("size-field");
    sizeField.addEventListener("input", event => {
        ...
        (<SVGElement><unknown>document.getElementById("svgMain")).setAttribute("viewBox", `0 0 ${currentSize} ${currentSize}`);
        refresh();
    });
    refresh();
}
...
function refresh() {
    const svgMain = <SVGElement><unknown>document.getElementById("svgMain");
    svgMain.innerHTML = "";
...
function drawCenterCircle(svgParent: SVGElement) {
    const svgCircleContainer = <SVGElement><unknown>document.getElementById("circle-container").cloneNode(true);
...
function polToCar(radius: number, angle: number, isRadians: boolean = false): { x: number, y: number } {
    if (!isRadians) {
        angle *= Math.PI / 180;
    }
...
function saveImage() {
    ...
    var canvas = <HTMLCanvasElement><unknown>document.createElement('CANVAS');
    ...
    img.onload = function () {
        ...
        ctx.lineWidth = 4;
        ctx.fillRect((canvas.width - svgWidth) / 2, (canvas.height - svgWidth) / 2,
            svgWidth, svgWidth);
...
function clearFields() {
    (<HTMLInputElement><unknown>document.getElementById("input-field")).value = "0";
    currentValue = 0;
    (<HTMLInputElement><unknown>document.getElementById("size-field")).value = "256";
    currentSize = 256;
    centerOffset = currentSize / 2;
    (<SVGElement><unknown>document.getElementById("svgMain")).setAttribute("viewBox", `0 0 ${currentSize} ${currentSize}`);
    refresh();
